...
{
    public const APP_VERSION    = '2026.07.13.2';
...
final class JmService
{
    ...
    private static function normalizeListItemPayload(mixed $item): array
    {
        ...
        $id = PayloadNormalizer::scalarString($item['id'] ?? $item['aid'] ?? $item['AID'] ?? '');
        if (trim($id) === '') throw new JmException('Invalid upstream list item id', 502);
