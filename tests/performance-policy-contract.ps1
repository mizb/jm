...
if ((Test-Area 'CacheList') -or $Area -eq 'Cache') {
    ...
    Assert-Matches $listItemNormalizer 'PayloadNormalizer::scalarString\(\$item\[''id''\]\s*\?\?\s*\$item\[''aid''\]\s*\?\?\s*\$item\[''AID''\][\s\S]*?trim\(\$id\)\s*===\s*''''[\s\S]*?Invalid upstream list item id' 'list item validator canonicalizes id aliases and rejects empty ids'
