...
The advanced reader optimization round plus homepage/weekly-list follow-up were implemented in API version `2026.07.07.7`. The current filter-fix maintenance target is API version `2026.07.13.1`. For future audit or follow-up maintenance, prefer the newer prompt at:
