...
环境变量默认：12 秒、15 次；允许 1～60 秒和 1～20 次的安全范围。`.3` 以已知可用老版为基线恢复每域最多三次与 300ms 生产重试间隔，wall budget 仍优先终止慢失败。
...
- 网络连接类错误在 `.2` 曾改为最多两轮五域快速轮询；进一步的老版可用对照证明该窗口仍偏窄后，`.3` 恢复同域最多三次、默认总尝试 15，并继续受 12 秒总 wall budget 限制；`.4` 追加真实 weekly type slug 兼容，不改变该预算策略。
- 首选域对 408/5xx 最多额外一次。
- 备用域一次。
...
powershell -ExecutionPolicy Bypass -File .\scripts\runtime-verify.ps1 -SkipBuild
```
...
```powershell
powershell -ExecutionPolicy Bypass -File .\scripts\performance-baseline.ps1 `
...
Get-FileHash -Algorithm SHA256 .\performance-evidence\fault-injection-runtime-docker.log
$health = Invoke-RestMethod 'http://127.0.0.1:8088/?health=1'
if ($health.diagnostics.test_mode -ne $false) { throw 'Production compose was not restored.' }
```
