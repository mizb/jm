...
- 每个上游调用有总尝试上限；`2026.07.17.8` 保持 `JM_MAX_UPSTREAM_ATTEMPTS=15` 和 12 秒 wall 硬上限，将健康排序后的域名按 `A→B→C→D→E` 最多执行三轮，普通瞬态故障只在轮次边界等待最多 300ms。weekly category ID 保持纯数字，真实 type ID 兼容 `hanman/another/manga` 一类安全 slug；章节、album、series、列表、搜索跳转和 weekly ID 兼容 JSON string/integer，角色校验前拒绝 bool/float/array，章节响应 ID 规范化后仍必须与请求 ID 完全一致；列表总数只接受可表示的非负整数，预取剩余 byte budget 必须进入单图 HTTP body collector。
- DNS、连接、TLS、timeout 及 HTTP 408/5xx 在本轮立即切换到下一健康排序域名；第二、三轮开始前最多等待 300ms。HTTP 429 继续遵守受剩余预算约束的 `Retry-After`；其他 4xx、JSON、解密、业务和 payload 错误立即停止。全部尝试始终同时受 15 次 attempt 与 12 秒 wall budget 限制。
...
| 故障 | 同域重试 | 切域 | 缓存 | 域名惩罚 |
|---|---:|---:|---:|---:|
| DNS/连接/TLS/timeout | 健康顺序逐域、最多三轮 | 本轮立即切域；下一轮前最多等待 300ms | 否 | 是 |
| HTTP 408/5xx | 健康顺序逐域、最多三轮 | 本轮立即切域；下一轮前最多等待 300ms | 否 | 是 |
| HTTP 429 | 等待后继续 | 合法 `Retry-After` 受剩余预算约束；无效值最多 300ms | 否 | 否 |
| HTTP 4xx | 否 | 否 | 否 | 否 |
