...
状态：**本机可执行的代码优化、测试、构建、元数据、真实 Suwayomi 回归和透明 HTTPS 同条件成功路径 BEFORE/AFTER 已完成。尚未闭环的是 Docker 多 worker 验收和生产签名。**

## 1. 结论与边界

- API 交付版本：`2026.07.13.2`。
- 扩展交付版本：`1.4.13`，`versionCode = 13`。
...
- 已从 Codex 原始补丁日志逆向恢复 pre-change 源码，并逐文件命中行为修改前记录的四个 SHA-256；新增获批的 loopback-only 透明 HTTPS harness 后，恢复版和当前版已在同一代理、fixture、PHP/APCu、单 worker、输入和样本量下完成成功路径 A/B。
- 正式 A/B 的主要结果是 fixture 上游请求 `397 → 7`（减少 `98.237%`），列表/专辑 p50 改善约 `21.550%～25.191%`；但 health 与图片 p50、图片 p95，以及 latest page 1/album 的部分 p99 有回退，不能概括成“所有指标都变快”。
...
| 文件 | SHA-256 |
|---|---|
| `jmcomic-api-main/index.php` | `680AF5970DD5EC8A85C2D6F75F66ECA439F91D1DBB3FF476406CB0096B18FB7C` |
| `jmcomic-api-main/Dockerfile` | `0FEA6C440407773253FB02A5ED647E588309047A3CC34F4DAFA7BE46876E03C7` |
| `jmcomic-api-main/docker-compose.yml` | `10EFA22E0E1735CD74BEF908CB5C7DAC6D72FA1F953A0855BBD1DCA97471C7DC` |
| `jmcomic-api-main/docker-entrypoint.sh` | `0A7054E0DABDEDEB3561D8312B5C75492986DD4580A4C7F2421043A8D6E758D9` |
...
| `tests/fixtures/upstream-router.php` | `9F7EC2D8E2A47A7CBCA9C9A4149B3DAB1BED4F07F4F60EC5C64FE2E22B2B331A` |
...
- APK：`D:\jm\jmapi-extension\dist-local\apk\tachiyomi-zh.jmapi-v1.4.13.apk`
- APK SHA-256：`9461671E153468CA3EB28111022818A4BDD2A82365DFA276A7E390310516FFA9`
- package：`eu.kanade.tachiyomi.extension.zh.jmapi`
- versionCode/versionName：`13 / 1.4.13`
- compile/target SDK：`37`
- v1/v2 签名：通过
- 签名证书 SHA-256：`c285ca414e833f61326e1104556114c4bb2db46686b4289576b2a758686ca080`
- `index.min.json` 与 `index.json` SHA-256：`7822EE350C6566D1B0BD2C98BA8B0D14C4DEC252F7DA39F9BB6D8C9B5D4BF1DB`
...
| PHP lint | 8/8，无语法错误 |
| PowerShell AST | 两项目 19 个 `.ps1`，0 错误 |
...
| Android 构建 | `spotlessApply` 与 `assembleRelease` 均 `BUILD SUCCESSFUL` |
| APK/元数据 | aapt2 manifest、v1/v2 签名、实际证书、3 个 JSON 回读、APK 名称/版本/哈希一致性全部通过 |
...
| 正式成功路径 A/B | PASS；warmup=10、iterations=120、concurrency=10；BEFORE/AFTER 各 600 个 warm 样本全部 status 200 且业务合同通过，`comparable=true` |
...
| Task 8 扩展修复 | 中文化、URL builder/basePath、ID/章节、预取双向同步合同通过；v1.4.13 构建、元数据、签名和 Suwayomi 实际请求通过 | 完成 |
| Task 9 版本与文档 | API 七处统一 `2026.07.13.2`；compose/README 所需 17 个环境变量 17/17；扩展 README/Gradle/APK/index 统一 v1.4.13/code 13 | 完成 |
...
- 恢复版 `index.php`：`A88271DCD759CDB4992FB2C49C963441FDA7028201E991211CD4DE4B497A13ED`；当前版：`680AF5970DD5EC8A85C2D6F75F66ECA439F91D1DBB3FF476406CB0096B18FB7C`。
...
- 恢复版配置源使用不遵循 cURL 代理的 `file_get_contents(https://...)`，因此两版均设置 `allow_url_fopen=0`：恢复版 fail-closed 后使用内置域名；当前版 cURL config 必须命中 loopback fixture。没有公网 fallback。
...
fixture 请求总数从 `397` 降到 `7`，减少 `390` 次／`98.237%`：恢复版 latest=262、album=131、chapter/scramble=2、media=2；当前版 config=1、latest=1、album=1、chapter/scramble=2、media=2。该差异直接证明列表源页和 album 短 TTL 缓存消除了绝大多数重复上游请求。

10 客户端图片队列探针两版均 10/10 成功，单 worker wall 为 `89.660ms → 95.053ms`；当前版没有改善该指标。它只描述 Windows 单 worker 排队，不是 Docker 多 worker singleflight 证据。
...
条件：PHP 8.3.32 built-in server、APCu 128M、memory limit 512M、实际 1 worker、无代理或 `JM_*` 环境变量；按当前版 A → 恢复版 BEFORE → 当前版 B 顺序请求同一个 latest 冷路径。

| 样本 | 源码 SHA-256 | HTTP | 客户端耗时 | 当前版诊断 |
|---|---|---:|---:|---|
...
这证明在本次相邻夹测中，恢复版失败路径慢于当前版两侧样本；但三次请求都因外部上游返回 502，网络也不可控。因此它只能作为失败路径原始事实，不能代替 deterministic fixture 成功路径、p95/p99、Docker 10-worker 或总体提升百分比。

## 6. 真实 Suwayomi 回归

环境：Suwayomi-Server `2.3.2243`，`http://127.0.0.1:4567`。

- 已安装扩展：`tachiyomi-zh.jmapi-v1.4.13.apk`，installed=true，hasUpdate=false。
- source ID：`3584941114123005289`。
- 当前偏好 Base URL：`http://127.0.0.1:18088/api`，证明反向代理子路径生效。
- 筛选实际返回：`排序 / 最新 / 最多浏览 / 最多点赞`。
- 设置实际返回：`JM API 地址`、`禁用 API 预取`，说明与摘要均为中文。
- Popular → `list=promote`；Latest → `list=weekly`。
- 空搜索排序 → `order=new/mv/tf`；标题搜索 `fixture` → `search=fixture&order=mr`。
- `JM350234`、album URL、纯数字均归一化为 `format=min&jmid=350234`。
- 首次详情：1 个 `?jmid`；刷新：1 个；10 路并发详情：10 个。证据不支持引入有生命周期风险的 APK 元数据缓存，继续由 API album cache 去重上游。
- 章节列表：1 个请求；返回 1 条章节、12 页；图片响应 HTTP 200、WebP、58 bytes。
- 预取开关 enabled → disabled → enabled：请求依次无 `prefetch=0`、含 `prefetch=0`、再次移除。
- API 回归后的预取 aggregate：events=3、scheduled=2、attempted=11、stored=10、bytes=1490；skip 只有 `disabled=1` 和 `budget-attempts=1`，没有 `executor-error`。

因此，中文筛选未生效不再是当前代码或 APK 问题；若设备仍显示英文，应核对宿主实际安装的 package/versionCode/签名以及扩展进程缓存，而不是重新修改筛选文本。
...
4. 2026-07-17 以无代理、无 `JM_*` 环境变量的独立生产进程完成当前 A／恢复版 BEFORE／当前 B 夹测：三者均 HTTP 502，耗时分别为 1149ms／6875ms／936ms；当前版两次均为 5 attempts 且 deadline=0。另一次当前版真实 latest 烟测为 1544ms、upstream 1493ms。故障均受预算约束，但外部上游可用性不是本仓库能修复的条件；该失败路径不能替代上节 loopback 成功路径 A/B，上节也不能外推为真实公网可用性或绝对延迟。
...
D:\jm\jmapi-extension\dist-local\
  apk\tachiyomi-zh.jmapi-v1.4.13.apk
...
- **同条件运行态 BEFORE/AFTER：本机 loopback 单 worker 成功路径已完成；Docker 多 worker和真实公网不在该结论范围。**
- **生产签名：发布治理待用户提供稳定密钥。**
...
> 完整读取并严格执行 `D:\jm\jmcomic-api-main\docs\performance-delivery-report.md` 及其中 2026-07-17 透明 HTTPS 设计/计划。先核对哈希；未变化时不要重做已通过的实现、A/B、APK 或 Suwayomi 回归。自主完成剩余的 Docker 多 worker/fault matrix；仅在用户提供稳定密钥后处理生产签名。遇到失败必须定位、最小修复并复测，直到完整交付；不得放宽安全/比较门禁，不得伪造 Git、BEFORE、签名或性能百分比。
